...
oidc_login_filter_allowed_values[0]="admins"
oidc_login_filter_allowed_values[1]="nextcloud_admins"
oidc_login_filter_allowed_values[2]="nextcloud_users"
...
mapfile -t CONFIG_SET < <(printf "%s" "$NEXTCLOUD_SYSTEM_CONFIG_SET")
for i in "${!CONFIG_SET[@]}"; do
    CONFIG_SET[i]="$(echo "${CONFIG_SET[i]}" | grep -Po '^[ \t]*\K.*[^ \t]')"
done

declare -a CONFIG_UNSET
mapfile -t CONFIG_UNSET < <(printf "%s" "$NEXTCLOUD_SYSTEM_CONFIG_UNSET")
