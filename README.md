...
[Original documentation](https://github.com/nextcloud/docker#persistent-data)
 ...
 - Custom apps are declared via `NEXTCLOUD_CUSTOM_APPS` and un-/installed automatically, no backup required
 - Configuration should be changed via environment variables declaratively and is not included in backups (see [List of environment variables](#environment-variables))
 - Main data at `/var/www/html/data` must be backed up via volume mounted from `${VOLUME_DIR}/nextcloud-data`
 - Themes are currently not backed up
